...
class _InvokeCallback(Protocol[_CT_contra]):
    async def __call__(self, __ctx: _CT_contra) -> None: ...

class _InvokeDecorator(Protocol[_CT]):
    @overload
    def __call__(self, func: _CT) -> _CT: ...
    @overload
    def __call__(self, func: _CoroType) -> _CoroType: ...
...
def max_concurrency(number: int, per: BucketType = ..., *, wait: bool = ...) -> Callable[[_F], _F]: ...
def before_invoke(coro: _InvokeCallback[_CT]) -> _InvokeDecorator[_CT]: ...
def after_invoke(coro: _InvokeCallback[_CT]) -> _InvokeDecorator[_CT]: ...
