...
- :attr:`Thread.members` is of type List[:class:`ThreadMember`] rather than List[:class:`Member`]
    ...
    - Most of the time, this data is not provided and a call to :meth:`Thread.fetch_members` to fetch the partial objects is needed.
...
- ``user_bot`` in :class:`~ext.commands.Bot`
